...
Receives raw Apple Health sleep sessions and HKWorkout sessions pushed from a device, aggregates them server-side, and writes formatted entries into the daily note. Receive-only; VaultEcho never pulls from a device. Send `{ "sleep": {...}, "workouts": [...] }`, or post a single sleep or workout object directly as the request body (it is detected by shape). Routing, target headings (or time-slot insertion), and the per-type line format come from the Apple Health Web UI settings; the format is a template with placeholders and conditional sections (`{{#field}}...{{/field}}`) that drop absent metrics. Each sleep session and each workout becomes one `[HH:mm]` entry that is merged and time-sorted under the configured heading (a night plus a nap are two sleep entries). Sleep sessions are attributed to the wake day and de-duplicated per session id (or fall-asleep time); workouts are de-duplicated by UUID.
...
**Ingest Apple Health Sleep**

Dedicated sleep endpoint: POST the sleep object directly as the request body (a `{ sleep: {...} }` wrapper is also accepted). Same parsing, wake-day attribution, per-session de-duplication, and configurable template as `health/ingest`.
...
```bash
curl -X POST http://localhost:8787/v1/api/health/sleep \
  -H "Authorization: Bearer change-me" \
  -H "Content-Type: application/json" \
  -d '{
    "id": "sleep-2026-06-17",
...
**Ingest Apple Health Workouts**

Dedicated workouts endpoint: POST a single HKWorkout object directly, or `{ workouts: [...] }` for several. Same formatting, per-UUID de-duplication, and configurable template as `health/ingest`.
...
```bash
curl -X POST http://localhost:8787/v1/api/health/workouts \
  -H "Authorization: Bearer change-me" \
  -H "Content-Type: application/json" \
  -d '{
    "uuid": "A1B2",
...
| Parameter | Description |
|---|---|
| `(body)` | A WeatherKit reading. Recognized fields (common export aliases accepted): `conditionText`/`condition`/`conditionCode` (a WeatherKit code like `MostlyCloudy`, or an already-localized string), `symbolName` (SF Symbol, used for the icon), `temperatureC`/`temperature` (°C; Fahrenheit also accepted), `apparentTemperatureC`/`apparentTemperature` (feels-like), `humidityPct`/`humidity` (0..100 or 0..1), `windKmh`/`windSpeed` (km/h), `uvIndex`, `precipitationChancePct`, optional `pressure`/`visibility`/`dewPointC`, `daylight`, the capture time (`capturedAt`/`date`/`asOf`), and an optional `id`. `weatherDate` (same ISO format) overrides the target day and time — use it to back-fill a past day from today. Placeholders: `{{time}}`, `{{date}}`, `{{icon}}`, `{{condition}}`/`{{conditionEn}}`/`{{conditionRaw}}`, `{{temp}}`, `{{feelsLike}}`, `{{humidity}}`, `{{precip}}`, `{{windSpeed}}`, `{{uvIndex}}`, `{{pressure}}`, `{{visibility}}`, `{{dewPoint}}`. |
