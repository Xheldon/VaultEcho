...
export const API_ENDPOINTS = [
  ...
  },
  {
    ...
    params: [
      ["sleep", "Optional. One sleep session: an array of stage samples, or an object `{ samples: [...], heartRate, hrv, respiratoryRate, wristTemperature, spo2, id }`. Send several sessions in one request (e.g. a night plus a nap) with `{ sessions: [ {...}, {...} ] }`; each becomes its own entry. Each sample needs `value` (HKCategoryValueSleepAnalysis number or identifier such as `asleepDeep`), `startDate`, and `endDate`. Vitals accept a single average number or an array of `{ value }` samples (heart rate also yields min/max). Provide a stable `id` per session for reliable de-duplication on re-push (otherwise the fall-asleep time is used). Available template placeholders: `wakeTime`, `bedTime`, `date`, `asleep`, `inBed`, `deep`, `core`, `rem`, `awake`, `latency`, `awakenings`, `avgHeartRate`, `minHeartRate`, `maxHeartRate`, `hrv`, `respiratoryRate`, `wristTemperature`, `spo2`, plus convenience groups `stages` and `vitals`."],
