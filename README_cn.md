...
```text
输入端 -> Coze 工作流 -> VaultEcho API -> 本地 Vault -> Obsidian Headless Sync -> Obsidian Sync
```
...
- `Allowed Top-Level Dirs`: 路径白名单，例如 `Inbox,Notes,Ideas,Projects,Daily,Reviews,Templates,Attachments,Archive`。
...
- 默认只允许写入 `Inbox`、`Notes`、`Ideas`、`Projects`、`Daily`、`Reviews`、`Templates`、`Attachments`、`Archive`。
